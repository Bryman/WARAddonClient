...
public class ActualAddon implements de.idrinth.waraddonclient.model.addon.Addon {
  ...
  public String getDescription(String language) {
    String description =
        "<p><strong>There is currently no Description for "
            + name
            + ".</strong></p>"
            + "<p>You can help by adding one at <a href=\"http://tools.idrinth.de/addons/"
            + slug
  ...
  public void fileWasChanged(File changedFile) {
    if (config.isEnabled(name)
        && changedFile.isFile()
        && changedFile.getName().equalsIgnoreCase(file)) {
      try {
  ...
  private class Updater {
    ...
    private java.io.File getZip() throws IOException {
      ...
      try (InputStream stream =
          client.getAddonDownload(slug + "/download/" + version.replace(".", "-") + "/")) {
    ...
    private boolean processDirectory() {
      for (java.io.File fileEntry : Objects.requireNonNull(folder.listFiles())) {
        if (!fileEntry.isDirectory()
            && org.apache.commons.io.FilenameUtils.getExtension(fileEntry.getName())
          ...
          try {
            ...
            if (description.getLength() > 0) {
              defaultDescription =
                  description.item(0).getAttributes().getNamedItem("text").getTextContent();
            }
            return true;
