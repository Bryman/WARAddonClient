...
public class Request {
  ...
  public Request(TrustManager manager, BaseLogger logger, Config config)
      throws NoSuchAlgorithmException, KeyStoreException, KeyManagementException {
    this.logger = logger;
    this.config = config;
    sslContext =
        org.apache.http.ssl.SSLContextBuilder.create()
            .loadTrustMaterial(manager.getKeyStore(), manager)
            .build();
  }
  ...
  public JsonObject getAddon(String slug) throws IOException {
    HttpResponse response =
        executionHandler(new HttpGet(config.getURL() + "addon-api2/" + slug + "/"));
    JsonReader reader = Json.createReader(response.getEntity().getContent());
  ...
  public java.io.InputStream getAddonDownload(String url) throws IOException {
    org.apache.http.HttpResponse response =
        executionHandler(
            new org.apache.http.client.methods.HttpGet(config.getURL() + "addons/" + url));
    return response.getEntity().getContent();
  }

  private synchronized org.apache.http.HttpResponse executionHandler(
      org.apache.http.client.methods.HttpRequestBase uri) throws IOException {
  ...
  public String getVersion() {
    ...
    try {
      HttpGet request =
          new HttpGet("https://api.github.com/repos/Idrinth/WARAddonClient/releases/latest");
